 ...
 * USB_DESC_SECTION removed so the array lands in .data (writable RAM). */
...
void usb_init(void)
{
     ...
     * Serial string descriptor data starts at byte offset 94 in the array:
     *   offset 92: bLength (0x22)
     *   offset 93: bDescriptorType (USB_DESCRIPTOR_TYPE_STRING)
     *   offset 94..125: 16 UTF-16 hex chars
     */
    {
        uint8_t chip_id[8];
        ...
        for (int i = 0; i < 8; i++) {
            rv_dap_plus_descriptor[94 + i * 4 + 0] = hex[(chip_id[i] >> 4) & 0x0F];
            rv_dap_plus_descriptor[94 + i * 4 + 2] = hex[ chip_id[i]       & 0x0F];
        }
    }
